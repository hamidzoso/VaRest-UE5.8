...
TArray<FString> UVaRestJsonObject::GetFieldNames() const
{
	...
	TArray<UE::JSON::Private::FJsonObjectSharedStringStorage::FStringType> TempKeys;
	JsonObj->Values.GetKeys(TempKeys);

	for (const auto& TempKey : TempKeys)
	{
		// با قرار دادن ستاره (*) قبل از متغیر، آنریل متن خام را استخراج و به FString تبدیل می‌کند
		Result.Add(FString(*TempKey));
	}
